...
class CancelTickets extends passengerDetails{
	void CancelTicket(){
		...
		System.out.println("Enter the Delete Pnr No:");
	...
	for(TicketNo=0;TicketNo<Name.length;TicketNo++){
				for(int Ticket=TicketNo+1;Ticket<Name.length;Ticket++){
					if(Seatno[Ticket]<Seatno[TicketNo+1]){
						temp = Seatno[Ticket];
						Seatno[TicketNo]=Seatno[Ticket];
						Seatno[Ticket]=temp;
					}
					}
				System.out.println(Seatno[TicketNo]);
